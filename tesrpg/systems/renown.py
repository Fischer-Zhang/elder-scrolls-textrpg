...
# fame → 聲望(renown)階 0..4 + 稱號(仿 crime._NOTORIETY_TITLES 階梯)
...
# 商人買價:每聲望階折扣 / 每惡名社交階加價(各夾 cap;by-design 旋鈕)
...
def renown_title(char: Character) -> str:
    """聲望稱號(由 renown tier;tier0 → 空字串)。與 crime.notoriety_title 並列。"""
...
def price_factor(char: Character) -> float:
    """商人**買價**乘數:聲望折扣 − 惡名加價。tier-0 → 1.0(no-op);只動 buy_price(賣價不碰,免 faucet)。"""
