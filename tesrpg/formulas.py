...
# (乘性·隨 block 技能縮放·同盾牆/重盾車道·元素穿透·**不碰命中**);**物理**被命中觸發 block 反擊樹
# (元素不觸發=鏡像 R42 荊棘·守 R127 物理牆)+ block XP。**無回合上繳**(代價=攻擊稅;上繳實測誘發
# 力竭螺旋+落架重立死迴圈 → sim 0% 全滅);力竭時姿態效果由 fatigue>0 閘整組暫停。
# 調參 sim 釘住(含立架耗一回合):tax0.75×M0.35(物理全額·元素折半)→ w1H gauntlet 86%
# (>w2H 84·基準 20)·true_dagon 21%·720 達貢 12%(⚠ 略超 R134 拍板包絡 0-8·已揭露)。
# 動任一值 → 必跑 sim_builds+sim_assassin。
GUARD_STANCE_DAMAGE_TAX = 0.75    # 姿態中攻擊傷害係數(走 damage_factor 車道·在偷襲倍率前 → solo 夾不變)
GUARD_STANCE_MITIGATION = 0.35    # 姿態卸力上限(block100 全額·隨 block/100 縮放)
