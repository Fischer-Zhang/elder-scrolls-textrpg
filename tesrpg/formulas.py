...
# R137 格擋姿態(取代整回合「格擋」動作·使用者兩拍板:w1H=續戰>2H·傷害<2H;**姿態不得產生落空**
# 〔荊棘反傷流預期常駐姿態·敵須打得中〕):姿態中照常攻擊但傷害×TAX;來襲**物理**傷害吃純減傷
# (乘性·隨 block 技能縮放·同盾牆/重盾車道·**物理全額·元素折半**·**不碰命中**);**物理**被命中觸發
# block 反擊樹(元素不觸發=鏡像 R42 荊棘;反擊傷套 resist_multiplier(physical) 守 R127 牆)+ block XP。
# **無回合上繳**(代價=攻擊稅;上繳實測誘發力竭螺旋+落架重立死迴圈 → sim 0% 全滅);
# 力竭時姿態效果由 fatigue>0 閘整組暫停。
# 調參 sim 釘住(含立架耗一回合·反擊套物抗後):tax0.75×M0.38(物理全額·元素折半)→
# w1H gauntlet 86.8%(n=500·>w2H 84·基準 20)·true_dagon ~11%·720 達貢 8.0%(n=1000·R134 包絡邊界)。
# 動任一值 → 必跑 sim_builds+sim_assassin。
...
# ① 元素卸力係數 ×0.70(一般盾 ×0.5):**R138b 使用者鐵令「現實邏輯優先——格擋不可能防元素
#    比物理多」**,推翻初版獨立 0.70×block 車道(元素卸 70% > 物理 38% 字面反轉)→ 改為
#    姿態卸力公式 ×0.70(block100 → 元素卸 ~26.6%·物理 38% 恆較高)。知情代價:重盾對
#    元素消耗戰回落(gauntlet 100→0%·火法王 100→36%·真身 44→0%)= 現實邏輯 > 平衡身份。
# ② 掩體回氣每回合 +2 體力:🔴 2 < 攻擊成本 ~4.2(雜技75)→ 邊打邊回不可能=力竭煞車仍咬合;
#    刻意不 gate fatigue>0(力竭時掩體喘息正是脫困路);站樁不打可無限回=零輸出雕像,自費時間。
GREAT_SHIELD_ELEMENTAL_FACTOR = 0.70  # 重盾姿態元素卸力係數(乘在姿態卸力上·一般盾 0.5)
...
WEAPON_PARALYZE_PROC = 0.10       # 武器麻痺觸發機率(1 回合、不重複套;solo BOSS 高機率抵抗 R44 → 反鎖王)
