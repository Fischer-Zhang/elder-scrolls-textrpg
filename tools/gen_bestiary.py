...
def flags(k, v):
    ...
    if v.get("summon"):    # R134/R135 BOSS 召喚爪牙:標明召什麼(固定 id 或隨機表)·wave/總量
        ...
        f.append(f"召喚:{what}×{sp.get('wave', 2)}(總{sp.get('cap', 6)})")
    if v.get("weight", 1) == 0:
...
- 不逃跑、不自我增益;法系/祭司怪會治療/護盾隊友(R87·換損攻擊);**帶召喚譜的 BOSS 會喚爪牙**(R134/R135:換損該回合攻擊 · 打完一波才召下一波 · 終身總量上限 · 殺王=爪牙潰散;爪牙無戰利品/擒魂/擊殺計數,不可復生取魂)。
