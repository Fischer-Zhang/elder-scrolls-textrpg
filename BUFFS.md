...
| 調息(R147·運動·主動戰鬥動作·非增益層) | 動作 | 戰鬥選單「😮‍💨 調息(耗一回合)」·體力未滿時提供:回體 = `rest_fatigue_amount(運動)`〔base 15+技能×0.15〕+ 里程碑 `rest_bonus`〔開源 25/75/100 = 8/12/16 SUM·max +36〕;**🔴 自動解除格擋姿態/盾牆**(恢復⇔持姿態互斥→防無限姿態)·耗一回合=不攻擊(零輸出=唯一成本閘)。運動樹二選一:開源(調息回更多)vs 節流(`fatigue_cost_bonus` 省體·SUM 夾 0.35·少需調息) |
