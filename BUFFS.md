...
| 十字軍聖壁 crusaders_ward | 抗性 | resist magic +30%;armor 24(R41 轉雙手重盾;★九神騎士團掌門;R142 更名與 R108 聖物 crusader_shield「十字軍聖盾」區隔) |
...
| 調息(R147·運動·主動戰鬥動作·非增益層) | 動作 | 戰鬥選單「😮‍💨 調息(耗一回合)」·體力未滿時提供:回體 = `rest_fatigue_amount(運動)`〔base 8+技能×0.15·非運動者僅 8〕+ 里程碑 `rest_bonus`〔開源 25/75/100 = 8/12/16 SUM·max +36〕;**🔴 自動解除格擋姿態/盾牆**(恢復⇔持姿態互斥→防無限姿態)·耗一回合=不攻擊(零輸出=唯一成本閘)。運動樹二選一:開源(調息回更多)vs 節流(`fatigue_cost_bonus` 省體·SUM 夾 0.35·少需調息) |
