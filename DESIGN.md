...
> **狀態:M0–M6 全部完成且可玩。** 測試模組數見 `python3 tests/run_all.py` 輸出結尾;每個里程碑收尾時跑過對抗式審查(workflow)。
...
- **技能數量** → 原設計採 **Oblivion 的 21 技能**(7 戰鬥 / 7 魔法 / 7 潛行,對應 7 屬性,Luck 不轄技能);後續經拍板**新增 `scout`(偵查,潛行系)與 `smithing`(鍛造)**,突破原上限以支撐「戰前偵查」與「鍛造」循環 → 目前為 **22**(當前數以 `len(gamedata.skills)` / `data/skills.json` 為準)。新增技能需同步 `progression.ensure_all_skills` 做舊存檔遷移。
- **一生評分** → 已實作公式(`systems/legacy.py`):等級 × 120 + 前五技能和 × 2 + 任務 × 50 + 公會階級積點 + 地城 × 90 + 足跡 × 20 + 擊殺 × 4 + 聲望 × 6 + 金幣 × 0.1 + 在世年數 × 30 → 換算稱號。

## 8. 之後可擴充(非當前範圍)
- ✅ ~~事件引擎(3.8)~~ **已於 M7 實作**:`data/events.json`(目前 45 事件)+ `systems/events.py`,在旅行/休息/探索/抵達依情境權重抽事件,選項含技能判定與需求閘門,效果複用既有系統(金幣/物品/技能/聲望/賞金/任務/戰鬥)。
