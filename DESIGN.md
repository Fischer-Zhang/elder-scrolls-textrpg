...
- **技能數量** → 採 **Oblivion 的 21 技能**(7 戰鬥 / 7 魔法 / 7 潛行,對應 7 屬性,Luck 不轄技能);**後續刺客大改經拍板新增第 22 技能 `scout`(偵查,潛行系)**,突破此上限以支撐「戰前偵查」循環。新增技能需同步 `progression.ensure_all_skills` 做舊存檔遷移。
