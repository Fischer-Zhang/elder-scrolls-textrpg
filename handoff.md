...
- **🚧 後續(階段三/未做)**:**佔領後收稅**(週期金幣源,複用 `tick_upkeep` 同款時間鉤子 → 與軍餉成收支閉環,城戰才有持續回報)/ 駐軍隨時間重建 / 自走 AI 陣營戰爭 / 攻下可安插自己為領主 / 親衛城/大義專屬募兵(具名將領,純改 JSON)。
...
> 戰績:十四輪審查累計修掉 **~28 個真 bug**、擋下 **2 個錯誤修法**、自補 1 次審查覆蓋缺口、自抓數個測試基建坑。
...
0. **城戰/領主區路線(已立藍圖,Oblivion+Skyrim 參考,逐 Phase 推進)** —— ✅ **Phase 1 已做**(見 §1「領主區 Phase 1」:第 4 城區 `領主區 👑` + 謁見領主,讓 21 城主活起來)。藍圖:
   ...
   - ✅ **招兵買馬 階段二已做**(見 §1「招兵買馬 階段二」):軍餉(`tick_upkeep` 週期金幣沉/付不出逃兵)+ 永久傷亡(`run_battle` casualties out-param → `apply_casualties` 名冊扣減,攻城陣亡不復生)+ 親衛複合來源(`warlord:true` 將領 veteran,唯營地可招)。把軍隊從「零成本永久常駐」修成「須維持、會折損」的資源。
     **🚧 後續(階段三/未做)**:**佔領後收稅**(複用同款時間鉤子 → 與軍餉成收支閉環)+ 駐軍重建 + AI 陣營戰爭 + 攻下自任領主 + 親衛城/大義專屬募兵。
...
> **招兵買馬 階段二**(§1:軍餉週期金幣沉/付不出逃兵 + 攻城永久傷亡(run_battle casualties 回報 → 名冊扣減)+ 親衛複合來源(warlord 將領 veteran);把軍隊修成須維持、會折損的資源;後續=佔領收稅閉環)。
>
