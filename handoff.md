...
- **狀態介面**:`status_line(state, gamedata, allies)` + web `_hud_view`/`renderHud` 一併顯示**夥伴(party 持久 HP/負傷)+ 召喚物(HP/回合)**;地城內每輪渲染 HUD。**修施法後敵狀態丟失**:`run_battle` 在「會改變戰況盤的玩家行動後」補繪 `combat_status_group`(`_BOARD_CHANGING_ACTIONS` gate)。
