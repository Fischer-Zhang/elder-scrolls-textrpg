...
- **daedric 韌性(後續修正)**:`_daedric_resurgence` 讓神話黎明大義打不死 —— kvatch_falls 後控城 < `DAEDRIC_FLOOR` 即「開湮滅之門」吞一城歸 daedric(決定性、不碰玩家城)。原本 daedric 入場僅 1 城、被吞即永久出局(`_aggressors` 排除 0 城者);復現後 sim 證跨 seed 0 局永久滅亡、且收斂不破。**調 `DAEDRIC_FLOOR`/`DAEDRIC_RESURGE_CHANCE` 必跑 sim_worldwar(daedric 參戰閘)**。
