...
- **🚧 Phase D ②+(後續)**:九神騎士團(聖道·lawful 對位,亦走 `unlock_event` 框架);其餘組織(大族入族/同伴/三聯/影鱗…)各自獨立一輪;更多大事件純加 world_events.json。
- **鐵律**:大事件易幟一律寫 `world_faction`(非 city_faction)→ 不污染稅基;玩家已佔城(city_faction)免疫事件易幟;加事件/旗號/大義純改 JSON。**`conjure_boon` 只在召喚分支、只對會員**;改 perk 數值改 `factions.json` per_rank/cap(純 JSON);新事件解鎖組織加 `unlock_event` 欄(通用,零碼)。

**內容量**:10 種族 / 13 星座 / 8 職業 / **22 技能(+偵查 scout)** / **19 武器(4 法杖)** / **34 護甲(7 材質整套)** / 25 法術(5 AoE) /
**15 材料(全部可野外採集/獵取)** / **4 飾品** / **製作配方系統(4 皮甲配方)** / **45 生物(7 高階 elite + 2 吸血鬼 + 5 黑沼澤 + 8 黑兄目標 + 7 神話黎明目標 + 2 heartland;18 隻帶 biome 生態標籤)** / 3 傭兵 / **36 地點(有環圖+生態 biome,世界閉合成大環;各省補全城市 賽8/天9/晨8/黑7/邊4,共 17 城+4 鎮)** / **6 地城** / **47 任務(3 分支壓軸 + 解咒 + 6 黑兄合約 + 6 神話黎明合約 + 10 在地任務含 2 任務鏈)** / **5 公會(+神話黎明,大事件解鎖)** / **7 開局背景** / **59 NPC(每城 3 / 每鎮 2,角色多樣、greeting + rumor 指路;8 名掛在地委託)** / **25 事件(含 10 省份限定;4 野採採集點)** / **吸血鬼化系統** / **黑暗兄弟會系統** / **技能里程碑系統(6 條 MVP,達門檻自動解鎖)** / **21 城主(各城自治)**。
