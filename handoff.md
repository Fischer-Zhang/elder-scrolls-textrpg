...
**機制(全部僅 姿態中+持雙手重盾 生效;非重盾玩家 byte-identical)**:① **元素掩體車道**:`_guard_stance_factor` 加 `gamedata` 參數(=None 走舊路徑=back-compat),持重盾時元素卸力改 `GREAT_SHIELD_ELEMENTAL_GUARD 0.70×block/100`+斯丹達爾之佑、夾 0.70(block100 → 元素只受 30%;一般盾維持 R137 元素折半;**隨 block 縮放=無投資施法者拿不到**,結構上封「法師持重盾白嫖元素牆」)。② **掩體回氣**:新 `combat.bunker_fatigue_regen(char,gd)`=唯一決策點(main 回合末 + `sim_builds._regen` 共用),姿態+重盾 → +`GREAT_SHIELD_BUNKER_FATIGUE 2` 體力/回;**🔴 2 < 攻擊成本 ~4.2(雜技75)→ 邊打邊回不可能、力竭煞車仍咬合**(專測鎖);**刻意不 gate fatigue>0**(力竭時盾後喘息=脫困路,防「歸零即永久死鎖」);站樁不打可持續回=零輸出雕像,自費時間非永動。③ **可見性(使用者鐵令)**:戰鬥選單持重盾時選項直標「重盾掩體（元素-N%·回氣+2/回)」(N 隨 block 動態)·立架訊息重盾專版·HUD 武器行加「卸力 X%·掩體:元素-N%·回氣+2」·codex 戰鬥篇加「重盾掩體」節。

**🔴 平衡(canonical `great_shield` build 入駐 sim_builds:魔族重盾+荊棘 daedric 5 件+姿態常駐)**:gauntlet 0→**100%**·火法王 2→**100%**·古龍/秩序騎士/削弱達貢 100%·**720 達貢 1.0%(n=400·R134 包絡內)**·無殭局(150 場:143 dead/5 timeout/2 win)。**⚠ 揭露:真身 true_dagon 0→44%(n=300)= melee 對真身新高**(assassin 40·w2H/盾反 20;70% 元素掩體正面吃下真身元素攻勢+荊棘磨物理混擊;未反轉 arcanist 69% 秘術牆·使用者知情)。**非重盾零位移**(w1H 720 9.3%≈基線 8.0 噪聲·盾反真身 20.5%=基線)。**sim_assassin BYTE-IDENTICAL**(隔離 worktree·刺客無重盾無姿態:元素車道 gate 於 `_wields_great_shield`·回氣 gate 於姿態 → 全不可達)。
...
**🔴 鐵律**:重盾掩體兩件套只在 姿態+重盾 生效(`_wields_great_shield` gate·勿放寬到一般盾 —— w1H+回血4 探針實測 720 達貢 8→56.5% 直接炸包絡);**`GREAT_SHIELD_BUNKER_FATIGUE` 必 < 攻擊成本**(否則無限全效姿態·test 鎖);回氣走 `bunker_fatigue_regen` 單一決策點(main+sim 共用·勿各寫);元素掩體隨 block 縮放勿改 flat(封施法者白嫖);動 0.70/2 → 必跑 sim_builds(gauntlet/火法王/720 包絡/真身)+sim_assassin(byte-identical);重盾階梯數值(卸力 5-8%·盾擊 6-12)未動——身份由掩體承載。前瞻:純重盾(無荊棘)gauntlet 34%=荊棘實質必配,若要「無荊棘也成立」再議;真身 44% 若嫌高 → 降 0.70 或真身曲目加物理混擊。
