...
**🔴 調參史(全 sim 釘)**:盾接機率版(50% 視同格擋)→ 使用者駁回(−15% 攻方命中=荊棘斷糧);純減傷物理限定 → 元素關卡變純稅(gauntlet 44%);全傷害卸力 → 終王越界(720 達貢 1→21%);物理全額+元素折半 M0.35 → gauntlet 86% 但反擊未套抗(720 達貢 12% 超包絡);**最終=+反擊套物抗、M 0.35→0.38 補回續戰**:w1H gauntlet **89%**(>w2H 84·基線 20)·knight/deathless 100%·**true_dagon 4→7.7%·720 達貢 0→7.7%(R134 包絡 0-8% 內 ✓·越界由反擊套抗解決非降 M)**。shield_reflect:設計**相容**常駐姿態(不減命中→荊棘照觸發=拍板要求),但實測其續戰 姿態 17%<基線 35%(輸出稅拖慢殺速→藥水經濟惡化)→ **canonical policy 不常駐**(`_STANCE_BUILDS={"warrior_1H"}`)·玩家可自選(對重物理 boss 情境有利)。

**驗證**:`run_all` **119**(新 test_guard_stance 5 測:切換/純減傷不碰命中〔命中序逐 seed 相同〕/力竭失效/姿態中反擊+XP/偷襲 solo 夾);**sim_assassin BYTE-IDENTICAL**(隔離 worktree·刺客雙持無姿態·非姿態 `_guard_stance_factor`=1.0 在常數讀取前短路·反擊套抗對既有怪 phys=0 → ×1.0 恆等);矩陣**只 w1H 行動**(true 4→7.7·720 0→7.7·gauntlet 20→89),其他 build 零漂移。
