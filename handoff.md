...
- **創角/出身/任務面板**:`ui.origins_panel`(創角前列各開局起始處境 + 起手任務)、`quest_log`/`_quests_view`(分組 origin/guild/委託 + 階段 ✔▶· 進度)、`character_sheet`/`_sheet_view`(出身列 + 起手任務)皆**雙渲染**;Web 端 `renderOrigins`+`VIEWS` 註冊、`renderQuests`/`renderSheet` 對應強化(`web/static/index.html`)。
